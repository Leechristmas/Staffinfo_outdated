	...
	BEGIN
		IF (SELECT COUNT(*) FROM DELETED) > 0
			...
			SELECT 'UPDATE', 'SPECIALITY', GETDATE(), 
				   'ОБНОВЛЕНА АТТЕСТАЦИЯ "' + (SELECT SPECIALITY FROM SPECIALITY E WHERE E.ID = DELETED.ID) + '"'
			FROM DELETED
		ELSE 
			INSERT INTO OPERATIONS_LOG(OPERATION_TYPE, TABLENAME, OPERATION_TIME, DESCRIPTION)  
			SELECT 'INSERT', 'SPECIALITY', GETDATE(), 
				   'ДОБАВЛЕНА АТТЕСТАЦИЯ "' + (SELECT SPECIALITY FROM SPECIALITY E WHERE E.ID = INSERTED.ID) + '"'
			FROM INSERTED
	END
GO
